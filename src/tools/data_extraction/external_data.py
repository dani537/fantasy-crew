...
from src.tools.data_extraction.auth import get_random_user_agent
    ...
    def parse_lineup_html(self, html_content: str) -> pd.DataFrame:
        ...
        for container_id, pos_name in position_map.items():
            ...
            for p in players:
                ...
                # If an alternative substitute is listed, create a complementary entry (100% - X%)
                if suplente:
                    try:
                        starter_pct = int(float(str(titularidad).replace('%', '').strip()))
                        sub_pct = max(0, 100 - starter_pct)
                    except ValueError:
                        sub_pct = 40
